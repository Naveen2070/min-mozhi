module PidKattu (
    ...
    assign __mimz_sub_5 = ($signed(__mimz_sub_4[(16)-1:0]) + (vikitha_veerrupaatu));
    ...
    assign pizhai = $signed(__mimz_sub_1[(9)-1:0]);
    assign vikitha_urruppu = $signed(__mimz_sub_2[(10)-1:0]);
    assign vikitha_veerrupaatu = $signed(__mimz_sub_3[(10)-1:0]);
    assign moththam = $signed(__mimz_sub_5[(16)-1:0]);
    assign kattuppaatu = $signed(__mimz_sub_6[(8)-1:0]);
    ...
    always @(posedge katikai) begin
        if (miittal) begin
            ...
            thokaiyiitu <= $signed(__mimz_sub_7[(16)-1:0]);
            munthaiya_pizhai <= $signed(pizhai[(8)-1:0]);
        end
    end
endmodule
