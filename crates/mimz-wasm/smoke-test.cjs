...
assert(
  verilog.includes("module Counter"),
  "expected `module Counter` in output",
);
...
assert(
  trace.includes("count"),
  `sim --trace should name the output, got:\n${trace}`,
);
...
assert(
  /\$var/.test(vcd) && /\$enddefinitions/.test(vcd),
  `sim --vcd should emit a VCD, got:\n${vcd}`,
);
...
assert(
  iface.clocked === false,
  `Add should be combinational, got: ${JSON.stringify(iface)}`,
);
assert(
...
assert(
  iface.outputs.some((p) => p.name === "sum"),
  "ports should list output sum",
);
...
const stepVcd = runCommand(adder, "sim", [
  "--steps",
  "a=3,b=5;a=7,b=1;a=0,b=2",
  "--vcd",
]);
assert(
  /\$var/.test(stepVcd),
  `sim --steps should emit a VCD, got:\n${stepVcd}`,
);
assert(
  /b10 /.test(stepVcd),
  `expected sum=2 (b10) from the last step, got:\n${stepVcd}`,
);
