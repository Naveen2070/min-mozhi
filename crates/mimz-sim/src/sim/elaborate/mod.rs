...
pub struct Design {
    ...
    /// Combinational `cover`s (module-item form) — tallied every settled
    /// comb state (GAP-6 follow-up). `on`-block covers need no separate
    /// field: they live inline in each `Process.body`, exactly like every
    /// other `SeqStmt`.
    pub covers: Vec<CoverStmt>,
}
...
fn assigns(body: &[SeqStmt], name: &str) -> bool {
    body.iter().any(|s| match s {
        ...
        SeqStmt::Cover(_) => false, // assigns no reg
