...
pub fn emit<'a>(project: &'a Project<'a>, files: &'a [File]) -> Result<String, Vec<Diag>> {
    let mut em = Emitter {
        project,
        out: String::new(),
        diags: Vec::new(),
...
struct Emitter<'a> {
    ...
    /// The CURRENT module's own `enum` declarations, by name — checked
    /// BEFORE `Project::resolve_enum`'s project-wide table by
    /// `Emitter::resolve_enum` (`module/ports.rs`). Two sibling modules in
    /// one file each declaring their own `enum State { .. }` (an ordinary,
    /// legal FSM pattern) both land in `Project::enums` under the same key
    /// `"State"`; `Project::resolve_enum` treats 2+ same-file candidates as
    /// a genuine ambiguity and returns `None` — wrongly, since each
    /// reference is completely unambiguous in ITS OWN module's scope. This
    /// mirrors `Checker::lookup_enum`'s module-scope-first algorithm
    /// (`checker/names/resolve.rs`), which the checker itself already
    /// uses for the exact same `Type::Named` lookup. Populated in
    /// `module()` from `m.items` (module-local enums only ever appear
    /// there, never nested), reset per module.
    cur_module_enums: HashMap<String, &'a EnumDecl>,
}
