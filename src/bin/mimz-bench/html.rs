...
pub fn render(report: &BenchReport, history: &[HistoryEntry]) -> String {
    ...
    let function_card = match &report.coverage.llvm {
        Some(l) => format!("{:.1}%", l.function_percent),
        None => "skipped".to_string(),
    };
    let testbench_card = match &report.accuracy.testbenches {
        Some(r) => format!("{:.0}%", r.percent()),
        None => "skipped".to_string(),
    };
    ...
    TEMPLATE
        ...
        .replace(
            "{{PEAK_RSS}}",
            &format!("{:.1} MB", report.memory.peak_rss_mb),
        )
        .replace(
            "{{GOLDEN_PCT}}",
        ...
        .replace(
            "{{FLAVOR_PCT}}",
            &format!("{:.0}%", report.accuracy.flavor_identity.percent()),
        )
        .replace("{{TESTBENCH_CARD}}", &escape(&testbench_card))
        .replace("{{FUNCTION_CARD}}", &escape(&function_card))
...
<div class="cards">
  ...
  <div class="card"><div class="label">Peak RSS (corpus)</div>
    <div class="value">{{PEAK_RSS}}</div></div>
  <div class="card"><div class="label">Golden match</div>
    <div class="value">{{GOLDEN_PCT}}</div></div>
  <div class="card"><div class="label">Flavor identity</div>
    <div class="value">{{FLAVOR_PCT}}</div></div>
  <div class="card"><div class="label">Error fixtures</div>
    <div class="value">{{FIXTURE_PCT}}</div></div>
  <div class="card"><div class="label">Testbenches</div>
    <div class="value">{{TESTBENCH_CARD}}</div></div>
  <div class="card"><div class="label">Code coverage (lines)</div>
    <div class="value">{{LLVM_CARD}}</div></div>
  <div class="card"><div class="label">Code coverage (fns)</div>
    <div class="value">{{FUNCTION_CARD}}</div></div>
</div>
...
<h2>Trends across benchmark runs</h2>
<div class="charts">
...
// Shared palette (GitHub-ish) so every chart speaks the same colors.
const C = {
  green: "#1a7f37", red: "#cf222e", blue: "#0969da", purple: "#8250df",
  orange: "#bc4c00", teal: "#1b7c83", pink: "#bf3989", gray: "#656d76",
};
// One x-axis label per run: short date + git rev.
const runLabels = DATA.history.map(h =>
...
// Code coverage: line/function/region breakdown bar (real llvm-cov), or the
// corpus-completeness doughnut when llvm-cov was skipped.
const covEl = document.getElementById("cov");
if (R.coverage.llvm) {
  const l = R.coverage.llvm;
  new Chart(covEl, {
    type: "bar",
    data: {
      labels: ["lines", "functions", "regions"],
      datasets: [{ label: "% covered",
        data: [l.line_percent, l.function_percent, l.region_percent],
        backgroundColor: [C.green, C.blue, C.teal] }],
    },
    options: {
      plugins: { title: { display: true, text: "Code coverage breakdown (cargo-llvm-cov)" },
                 legend: { display: false } },
...
// Line: validation rates across runs. All series share one 0–105 % axis —
// the 105 max keeps the always-100% lines off the top edge (the old dual-axis
// version pinned golden + fixtures at the chart border, where they overlapped
// and vanished). Distinct colors + point markers + legend toggles make even
// coincident 100% lines legible. Older history lines lack some fields → null →
// spanGaps draws across the gap.
const rateLine = (label, key, color) => ({
  label, data: DATA.history.map(h => h[key] ?? null),
...
new Chart(document.getElementById("trend"), {
  ...
  data: {
    labels: runLabels,
    datasets: [
      rateLine("golden match (%)", "golden_pct", C.green),
      rateLine("error fixtures (%)", "fixture_pct", C.red),
      rateLine("flavor identity (%)", "flavor_identity_pct", C.blue),
      rateLine("no false positives (%)", "clean_pct", C.orange),
      rateLine("help lines (%)", "help_pct", C.pink),
      rateLine("code coverage (lines %)", "llvm_line_pct", C.teal),
    ],
  },
  options: {
    plugins: { title: { display: true, text: "Validation rates across runs" } },
    interaction: { mode: "index", intersect: false },
    scales: { y: { min: 0, max: 105, title: { display: true, text: "%" } } },
  },
});

// Line: pipeline time across runs (ms) — its own axis, no longer fighting the
// rate lines for a shared scale.
new Chart(document.getElementById("time-trend"), {
  type: "line",
  data: {
    labels: runLabels,
    datasets: [
      { label: "pipeline total (ms)", data: DATA.history.map(h => h.total_ms),
        borderColor: C.purple, backgroundColor: C.purple, pointRadius: 3, tension: 0.2 },
    ],
  },
  options: {
    plugins: { title: { display: true, text: "Pipeline time across runs (ms)" } },
    scales: { y: { type: "linear", beginAtZero: true, title: { display: true, text: "ms" } } },
  },
});

// Line: throughput trend (LOC/s).
new Chart(document.getElementById("speed"), {
  type: "line",
  data: {
    labels: runLabels,
    datasets: [
      { label: "Throughput (LOC/s)", data: DATA.history.map(h => h.loc_per_sec),
        borderColor: C.green, backgroundColor: C.green, pointRadius: 3, tension: 0.2 },
    ],
  },
  options: {
...
new Chart(document.getElementById("memory"), {
  ...
  data: {
    labels: runLabels,
    datasets: [
      { label: "Peak RSS (MB)", data: DATA.history.map(h => h.peak_rss_mb ?? null),
        borderColor: C.purple, backgroundColor: C.purple, pointRadius: 3, tension: 0.2, spanGaps: true },
    ],
  },
  options: {
    plugins: { title: { display: true, text: "Peak memory trend (RSS, MB)" } },
    scales: {
      y: { type: "linear", beginAtZero: true, title: { display: true, text: "Megabytes" } },
    },
  },
});

// Run details table — built from the embedded report meta (no placeholders).
(() => {
  const m = R.meta;
  const rows = [
    ["Generated", new Date(m.timestamp_ms).toLocaleString()],
    ["Git revision", m.git_rev],
    ["Compiler", m.rustc],
    ["Timing iterations", String(m.iterations)],
    ["Example files", String(m.example_files)],
    ["Error fixtures", String(m.fixture_files)],
    ["Code coverage", R.coverage.llvm
      ? `${R.coverage.llvm.line_percent.toFixed(1)}% lines, `
        + `${R.coverage.llvm.function_percent.toFixed(1)}% functions, `
        + `${R.coverage.llvm.region_percent.toFixed(1)}% regions`
      : (R.coverage.llvm_note || "skipped")],
  ];
...
mod tests {
    ...
    fn report_renders_a_complete_page() {
        ...
        assert!(
            html.contains("Validation rates across runs"),
            "the fixed rates trend (golden + fixtures + more, single % axis)"
        );
        assert!(
            html.contains(r#"id="time-trend""#),
            "the split-out pipeline-time trend canvas"
        );
        assert!(
            html.contains(r#"id="run-details""#),
            "the run-details table"
        );
