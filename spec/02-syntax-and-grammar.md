...
- All modules and enums of the imported file come into scope. Module,
  ...
  `a.b.Name` (see section 1.5b). Function names remain project-wide unique
...
- `extern module` names are unique **within one file**, same rule as
  ...
  import path if it becomes ambiguous, section 1.5b).
...
representation, just sugar over the bundle machinery `section 1.12` already
describes.
...
- Because `T?` desugars to an ordinary structurally-typed bundle, a
  ...
  `section 1.12`'s structural matching rule, not a special case carved out for
  this feature.
...
**Verilog emission:** `T?` flattens exactly like any other bundle (`section 1.12`)
...
5. **Domain typing:** `clock`/`reset` never appear in data expressions; data
   ...
   synchronizer primitives (section 1.2b).
...
| Handshake (req/ack) protocols, async FIFOs    | future work (stdlib, on top of section 1.2b)                         |
| Buses (multi-signal channels beyond `bundle`) | post-Phase 2 (stdlib time) - plain `bundle`s ship now (section 1.12) |
...
dst_clock)` is a toggle-based synchronizer for a single-cycle pulse; both
  ...
  diagnostic E1116 (unknown `sync.` method name). section 6 rule 5 and the section 7
...
- **v0.2.19 (2026-07-01):** **Packages / namespacing** (backlog:
  ...
  narrows from project-wide to per-file (section 1.5). New qualified-reference
  syntax `a.b.Name` (section 1.5b) at 4 reference points, reusing the `import`
