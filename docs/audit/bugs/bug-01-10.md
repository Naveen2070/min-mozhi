# Functional bugs - BUG-01 to BUG-10
...
## BUG-1 (HIGH, FIXED 2026-06-13) - Stray top-level `}` spun the parser into an OOM hang

**Status:** FIXED 2026-06-13 (commit `e519690`) - pre-dates this file's later
`(SEVERITY, STATUS)` heading convention; backfilled 2026-08-18 (round-7 plan
Task 8's own "no index row contradicts a verified disposition" standard,
applied here since the index carried a stale `-` for a bug this entry's own
`**Fix.**`/`**Test.**` sections already show closed).
...
## BUG-2 (LOW, FIXED 2026-06-15) - `translate` reskin glued a number onto a Tamil token into unlexable output

**Status:** FIXED 2026-06-15 (commit `73051bc`) - backfilled 2026-08-18, same
reason as BUG-1 above.
...
## BUG-3 (HIGH, FIXED 2026-06-21) - `--emit-testbench` dropped module parameter defaults, breaking width resolution

**Status:** FIXED 2026-06-21 (commit `a27b12c`) - backfilled 2026-08-18, same
reason as BUG-1 above.
...
## BUG-4 (MEDIUM, FIXED 2026-06-21) - Test names sanitizing to the same Verilog identifier silently collided

**Status:** FIXED 2026-06-21 (commit `a27b12c`) - backfilled 2026-08-18, same
reason as BUG-1 above. The follow-up note below (pure-Tamil test names) is a
separate, still-open DX improvement, not a correctness gap.
...
## BUG-5 (LOW, FIXED 2026-06-22) - `translate` romanize glued `0b…?` (MaskedInt) onto a romanized identifier, breaking re-lex

**Status:** FIXED 2026-06-22 (commit `723f05f`) - backfilled 2026-08-18, same
reason as BUG-1 above.
...
**Fix - returns, diagnostic (2026-07-18, interim - per the
correctness-consolidation plan, Stage 1).** Until
