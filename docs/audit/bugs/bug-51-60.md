...
(round-4 class-closure plan, Task 4) - the
...
**How found.** Round-4 class-closure plan Task 4 - verifying the
`Int`/`Bool` arms' "nothing to compare" claim as a checkable
...
"the checker rejects it upstream" claim (round-4 class-closure plan, same
task as BUG-56). The claim is false - `mimz check` accepts an array
...
**What.** Found continuing Task 4's audit (round-4 class-closure plan),
...
(`MIMZ_DIFF_FUZZ_N=5000 MIMZ_DIFF_FUZZ_CLOCKED_N=5000`, round-4 class-closure
plan Task 1) for the first time
...
([`docs/guide/05-operators.md`](../guide/05-operators.md) section Reductions teaches
the three prefix reductions) applied to a chapter-6 builtin, at a plain
assignment. Four shipped examples use a reduction
(`examples/{english,tanglish,tamil,mixed}/bitops.mimz:21-23` - `nand(a)`,
`nor(a)`, `xnor(a)`), but none gives it a **narrow-rendering operand**,
which is why the golden suite never saw it (round-6 review Part 4.3,
`docs/audit/review-2026-08-15.md`, corrected here - this file's own
