...
| [`review-2026-08-18.md`](review-2026-08-18.md) | v0.2.0-rc1 readiness round 8 - **no-go**, a deliberately light-touch four-check round (each of the five round-7 fixes has its own passing repro; the new declaration-order invariant fires when deliberately broken; corpus and testbench sweep clean; gate 5 at 5000/5000) rather than a full adversarial re-sweep; items 2/4 (diagnose-not-panic, BUG-67) verified fixed on a release build, item 3 fixed for its own three repros, corpus clean 226/50/90/0, gate 5 clean at 5000/5000 - but item 1's declaration-order invariant is shown to be **name-specific, not axis-level** (broken on two constructed cases, `BUG-70`), and item 5's own filed repro still fails (`BUG-71`, `emit_testbench` never folds file-level consts into its env)                                                                                                    |
| [`review-2026-08-20.md`](review-2026-08-20.md) | v0.2.0 release verification round 9 - **go**, 8 of 8 gate items; BUG-70's three constructions, BUG-71's three filed repros, BUG-72 and BUG-74 all independently reproduced; **GAP-18 verified genuinely closed** (widened invariant fires on a faithful Task-1 revert naming identifier and both lines, silent across all 226 files, survives independently constructed traps in every category), Task 9's fuzz criterion re-derived and GAP-20's deferral checked against `mimz-sim`'s own E0201; corpus 226/226 elaborating, 90/90 testbench modules PASS, 33 goldens confirmed pure `#0` reversions, gate 5 clean at 5000/5000, suite 1314/1314. Three documentation defects found and corrected (a load-bearing guard documented as unreachable, two stale "BUG-73 not fixed" notes, an over-broad known-issues framing) - **no behaviour defect** |
