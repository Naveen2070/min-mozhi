...
Reading files off a physical SSD/HDD introduces heavy statistical noise. We only want to measure the *compiler's* parsing and checking speed, not the operating system's disk read speed.

**Implementation Strategy:**
- Implement a "Warm-Up" pass in `mimz-bench`. 
- Before starting the `std::time::Instant` timer, execute the entire benchmark once *without recording the time*.
