...
| **IR**              | 2       | ✅ Typed netlist (`mimz_core::ir`): bit-addressable `NetId`/`Bits`, 30 `CellKind`s (arithmetic/logic/compare/mux/concat/slice/`Dff`/`Mem`/`BlackBox`/`Const`); `Design` → `Module` lowering (registers → `Dff`, memories → `Mem`, `fn` calls inlined, `extern module` → `BlackBox`); line-based + s-expr (dump-only) text formats; 5-check validation pass (drivers/undriven/widths/comb-cycles/black-box shape); own executor (`ir/exec.rs`) for differential testing against the AST-level simulator; a per-value `signed` flag on `Bits` (added 2026-09-10, see Decision log) makes `min`/`max`/`abs`/`extend` and comparison signedness exact by construction instead of a best-effort heuristic; `foreach`/`loop` inside a `fn` body or `on`-block fully unrolls at lowering time via const-var-substitution (added 2026-09-22, Task 7) — no residual loop construct reaches `ir::Module`. Optimizer passes not yet built |
...
render_diags_lang};`). Since Phase 2 Step 0 it also owns `elaborate`
  (AST → flat `Design`), `value` (the `Val` bit-vector model), and `comb`
  (the combinational expression evaluator) - `elaborate` was already
  structurally pure, so promoting it (plus the `value`/`comb` it folds
  constants through, and the S0xxx runtime-diagnostic catalog in `diag.rs`)
  lets a future `mimz-core::ir` module consume the same `Design` type
  without depending on the simulator crate.
- **`mimz-sim`** - the event-driven kernel (`sim/kernel.rs`) and supporting
  modules (`harness`, `run`, `vcd`, `trace`, `host`, `wide`) plus the
  in-memory command runner (`runner.rs`), depending only on `mimz-core` -
  including for its `Design` input, via `mimz-core::elaborate` (no longer
  owning elaboration itself; `sim/mod.rs` and `sim/diag.rs` re-export
  `elaborate`/`value`/`comb`/the S0xxx catalog for zero caller churn).
  Defines the `EmulationHost` trait + `Direction` enum (`sim/host.rs`) so
  the simulator can drive hardware peripherals without depending on them.
...
│   │       ├── diag.rs                    # teaching diagnostics + JSON format (E-code checker catalog + S0xxx sim runtime catalog)
...
│   │       ├── value/                     # bit-vector value model (4 files)
...
│   │       │   ├── sync_prim_lower.rs         # sync CDC double_flop/pulse lowering
│   │       │   ├── foreach_lower.rs           # foreach desugaring
│   │       │   └── builtin_bundles.rs         # valid-bundle ? / ?? synthetic bundles
│   │       ├── lexer/                     # E10xx
...
│   │       │   ├── items/                     # file/module/inst/seq/test/func/bundle/extern items
│   │       │   ├── expr.rs                    # precedence climbing, patterns
│   │       │   └── tests/                     # unit tests, split by topic (14 files)
│   │       ├── emit_verilog/
│   │       │   ├── mod.rs                     # Project symtab, entry, helpers
│   │       │   ├── module/                    # ports, drives, seq, instances, funcs (8 files)
│   │       │   ├── expr.rs                    # expression rendering
│   │       │   ├── kinds.rs                   # self-determined kind classification
│   │       │   ├── self_determined.rs         # self-determined position checking
...
│   │       └── sim/                       # (P1.5)
│   │           ├── mod.rs                     # module entry; re-exports elaborate/value/comb from mimz-core
│   │           ├── kernel.rs                  # event-driven kernel
│   │           ├── diag.rs                    # shim: re-exports ALL_SIM_CODES/bridge_code/diag_from_bridged from mimz-core::diag
│   │           ├── harness/                   # test block runner (2 files)
│   │           ├── host.rs                    # EmulationHost trait + Direction enum
│   │           ├── run.rs                     # default stimulus
│   │           ├── wide.rs                    # arbitrary-width sim helpers
...
│   ├── lsp.rs                             # `mimz lsp` server (BIN-only module, keeps tokio out of the lib)
│   ├── bin/mimz-bench/                    # benchmark harness (docs/code/12): main.rs / metrics/ / html.rs
...
│   ├── examples.rs                        # all 200 examples (english/tanglish/tamil: 44 each, mixed: 43, tamil-pure: 25)
...
│   ├── differential_fuzz.rs               # random-program differential fuzz vs Icarus (v1+v2+v3: unsigned/signed/clocked)
│   ├── self_determined_regression.rs      # BUG-19/BUG-20 self-determined-position hoisting regressions
│   ├── extern.rs                          # extern module (Verilog FFI) CLI/config plumbing
...
├── examples/                   # .mimz programs (english/tanglish/tamil: 44 each, mixed: 43, tamil-pure: 25 = 200)
