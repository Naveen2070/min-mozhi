...
| `our_simulator_matches_icarus_bit_for_bit`  | **Layer 3 (B8):** three views must agree bit-for-bit per cycle — our event-driven kernel (in-process), the VCD waveform our writer emits, and Icarus running the emitted Verilog under the same generated stimulus (counter + shift register + edge detector). Where Layer 2 checks Icarus against hand-written asserts, this pits our simulator (engine AND waveform) directly against Icarus |
...
## Integration: sim (`tests/sim.rs`, 7 tests — run the real binary + lib in-process)

End-to-end `mimz sim` over a counter: the default stimulus, the VCD, and the
console trace; plus the B8 kernel perf baseline and the golden VCD byte-lock
(both run the lib in-process).
