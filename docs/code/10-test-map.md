...
## Unit: sim runner / VCD / console trace (`src/sim/{run,vcd,trace}.rs`, 9 tests)

Phase 1.5 step B4/B5: the default stimulus + timeline capture (`run.rs`), the
hand-written 2-state VCD writer (`vcd.rs`), and the console trace renderer
(`trace.rs`) — all over one per-cycle snapshot from the kernel.
...
| `a_clockless_module_is_rejected` (run)         | a module with no clock errors (pointing at `mimz eval`)                             |
...
## Integration: sim (`tests/sim.rs`, 5 tests — run the real binary)

End-to-end `mimz sim` over a counter: the default stimulus, the VCD, and the
console trace.
...
| `a_clockless_module_is_rejected_with_a_pointer_to_eval` | a combinational module fails sim, pointing at `mimz eval`                 |
...
| `fmt`, grammar engine, full simulator                   | mostly built: all five word-order flips ship now (`syntax thamizh` + clocked-block, conditional, if-expression, match, and the **test** flip — `tests/grammar.rs`, `tests/test_run.rs`); `translate --order` and the event-driven kernel (`mimz sim` / `mimz test`) ship too. Remaining 1.5 work: B8 (Icarus differential + perf baseline) | with their phases (1.8 / 1.5)                               |
