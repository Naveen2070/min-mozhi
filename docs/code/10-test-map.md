...
## Integration: translate (`tests/translate.rs`, 12 tests — the four-flavor oracle + the `--order` pretty-printer + `--romanize-names` + the sidecar name-map)
...
## Integration: config (`tests/config.rs`, 4 tests — run the real binary)
