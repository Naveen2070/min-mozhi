...
## Integration: test (`tests/test_run.rs`, 5 tests — run the real binary)
...
| `fmt`, grammar engine (remaining flips), full simulator (clocked kernel) | partly built: `syntax thamizh` + the clocked-block, conditional, if-expression, and match flips ship now (`tests/grammar.rs`); the **test** flip (deferred to 1.5), `translate --order`, and the event kernel remain | with their phases (1.8 / 1.5)                               |
