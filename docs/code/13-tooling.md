...
## `explain` (`crates/mimz-core/src/explain.rs`) — `mimz explain <CODE>`
...
## `translate` (`crates/mimz-core/src/translate.rs`) — `mimz translate --to <flavor>`
...
## `morph` (`crates/mimz-core/src/morph.rs`) — error-language selection + Tamil inflection
...
## `sim` (`crates/mimz-sim/src/sim/`) — `mimz eval` / `mimz sim` / `mimz test` (Phase 1.5, complete)
...
## `version` (`crates/mimz-core/src/version.rs`) — the two version axes
...
## `analysis` (`crates/mimz-core/src/analysis.rs`) + the LSP editor features
...
- **`crates/mimz-core/src/analysis.rs` (library, pure, async-free)** — reusable by the future
