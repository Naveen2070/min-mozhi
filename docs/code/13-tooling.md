# 13 — Tooling modules (`explain`, `translate`, `pretty`, `sim`)
...
word-order flips (the test-form flip + the morphology error catalog are still
ahead), and `sim::comb` is combinational only (the kernel is Phase 1.5). Each
