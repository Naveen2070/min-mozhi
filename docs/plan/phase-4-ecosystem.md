...
- [~] VS Code extension: syntax highlighting (all flavors + thamizh-order), inline diagnostics via LSP — **LSP v0 (diagnostics only) pulled into Phase 1** (Decision 2026-06-12); this phase adds hover, go-to-definition, completion, and `translate` integration on top. **Hover / go-to-definition / completion DONE 2026-06-25** (branch `phase-4-lsp-dx`): pure `src/analysis.rs` symbol index + `resolve_at` + `completions` over `parse_recover` partial trees, three `src/lsp.rs` handlers + doc-text store + capabilities; cross-file in-tree resolution incl. `test` blocks; trilingual majority-flavor keyword completion. Deferred: dot-member completion, flavor-localized hover render, `translate` integration
